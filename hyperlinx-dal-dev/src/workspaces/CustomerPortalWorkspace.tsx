...
type ProjectTab = "Overview" | "Proposal" | "Engineering" | "Service Order" | "ScopeVersion" | "Specifications" | "Documents" | "Activity";
...
export default function CustomerPortalWorkspace() {
  ...
  const isDemo = session?.user.authorityClass === "DEMO";
  ...
  return (
    <div className="customer-portal-shell">
      ...
      {isDemo ? <div className="customer-portal-demo-banner">DEMO ENVIRONMENT · CUSTOMER VIEW · NOT PRODUCTION ELIGIBLE</div> : null}
      ...
      {section === "Projects" ? <main className="customer-portal-main">
        ...
        <section className="customer-project">
          {selected ? <>
            ...
            {selectedDeal ? <section className="customer-artifact-history" aria-label="Governed artifact history">
              <span>Proposal<strong>{selected.artifactStates.proposal.state.replaceAll("_", " ")}</strong></span><i>→</i>
              <span>Engineering<strong>{selected.artifactStates.certifiedIof.state === "CERTIFIED" ? "CERTIFIED" : selected.artifactStates.engineering.state.replaceAll("_", " ")}</strong></span><i>→</i>
              <span>Service Order<strong>{selected.artifactStates.serviceOrder.signatureState === "FULLY_SIGNED" ? "FULLY SIGNED" : selected.artifactStates.serviceOrder.state.replaceAll("_", " ")}</strong></span><i>→</i>
              <span>ScopeVersion<strong>{selected.artifactStates.scopeVersion.state.replaceAll("_", " ")}</strong></span>
            </section> : null}
            {selectedDeal ? <section className="customer-state-control" aria-label="Current state action">
              {selectedDeal.currentState === "CUSTOMER_REVIEW" ? <><b>Customer Review</b><button className="primary" onClick={() => setTab("Proposal")}>Accept Proposal / Request Changes</button></> : null}
              {selectedDeal.currentState === "ACCEPTED" ? <b>Accepted — Engineering handoff underway</b> : null}
              {selectedDeal.currentState === "ENGINEERING" ? <><b>Engineering review underway</b><button onClick={() => setTab("Engineering")}>View Engineering Status</button></> : null}
              {selectedDeal.currentState === "CERTIFIED" ? <><b>Engineering certified</b><button onClick={() => setTab("Service Order")}>View Certified Scope / Service Order</button></> : null}
              {selectedDeal.currentState === "SERVICE_ORDER" ? <><b>Service Order ready</b><button className="primary" onClick={() => setTab("Service Order")}>Sign Service Order</button></> : null}
              {selectedDeal.currentState === "CUSTOMER_SIGNED" ? <b>Awaiting Teralinx Countersignature</b> : null}
              {selectedDeal.currentState === "COUNTERSIGNED" ? <b>Authorization processing</b> : null}
              {selectedDeal.currentState === "AUTHORIZED" ? <><b>Execution scope authorized</b><button className="primary" onClick={() => setTab("ScopeVersion")}>View ScopeVersion</button></> : null}
            </section> : null}
            ...
            <div className="customer-project-tabs">{(["Overview", "Proposal", "Engineering", "Service Order", "ScopeVersion", "Specifications", "Documents", "Activity"] as ProjectTab[]).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{item}</button>)}</div>
            ...
            {tab === "Proposal" && selectedDeal ? <article className="customer-governed-document customer-proposal-card">
              <header><div><small>TERALINX · GOVERNED PROPOSAL</small><h3>{selected.title}</h3><span>{accountTwin?.account.name}</span></div><div><b>Revision {selected.proposal.proposalRevisionNumber}</b><span>{selected.artifactStates.proposal.state.replaceAll("_", " ")}</span></div></header>
              ...
              {selectedDeal.currentState === "CUSTOMER_REVIEW" && canReview ? <><label className="customer-message">Question, comment, or requested change<textarea value={message} onChange={(event) => setMessage(event.currentTarget.value)} placeholder="Add context for the Teralinx team" /></label><div className="customer-actions"><button onClick={() => void act("questions", { message })}>Ask Question</button><button onClick={() => void act("change-requests", { ...exactProposal, comment: message })}>Request Change</button><button className="danger" onClick={() => void act("proposal/decline", { ...exactProposal, comment: message })}>Decline</button><button className="primary" onClick={() => void act("proposal/accept", { ...exactProposal, comment: message })}>Accept Proposal</button></div></> : <p className="customer-readonly">{selected.artifactStates.proposal.state === "ACCEPTED" ? "This exact Proposal Revision was accepted and is retained as historical truth." : "This perspective is read-only."}</p>}
            </article> : null}
            {tab === "Engineering" && selectedDeal ? <article className="customer-proposal-card"><h3>Engineering Status</h3><div className="customer-document-grid"><span>Engineering package<strong>{selected.artifactStates.engineering.state.replaceAll("_", " ")}</strong></span><span>Eligibility<strong>{selected.artifactStates.engineering.eligibility.replaceAll("_", " ")}</strong></span><span>Certified IOF<strong>{selected.artifactStates.certifiedIof.state.replaceAll("_", " ")}</strong></span></div><p>{selected.engineering.customerSafeSummary}</p></article> : null}
            ...
            {tab === "ScopeVersion" && selectedDeal ? <article className="customer-proposal-card"><h3>ScopeVersion — Order for Execution</h3>{selected.scopeVersion && selected.artifactStates.scopeVersion.state === "AUTHORIZED" ? <div className="customer-document-grid"><span>Status<strong>AUTHORIZED</strong></span><span>ScopeVersion<strong>{selected.scopeVersion.scopeVersionId}</strong></span><span>Created<strong>{selected.scopeVersion.createdAt ? new Date(selected.scopeVersion.createdAt).toLocaleString() : "Persisted"}</strong></span></div> : <p>ScopeVersion remains unavailable until the complete customer-signature and Teralinx-countersignature chain authorizes execution.</p>}</article> : null}

...
export default function CustomerWorkspace() {
  ...
  async function sendToEngineering() {
    ...
    setStatus("Resolving the exact accepted Proposal through the governed Draft IOF handoff...");
    try {
      ...
      const result = await submitDraftIofPackageToEngineering(draft.packageId, {}, session);
      setTwin(await loadAccountCustomerTwin(accountId, session));
      setStatus(`Engineering handoff accepted: ${result.engineeringPackage.engineeringPackageId}.`);
    } catch (error) {
      setStatus(`Engineering handoff blocked: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
